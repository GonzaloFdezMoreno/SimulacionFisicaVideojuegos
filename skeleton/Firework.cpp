...
Firework::Firework(Vector3 pos, Vector3 vel, float damping, Vector3 accelerate, Vector4 color, int tam):Particle(pos,vel,damping,accelerate,color,tam,0,false) {
...
std::list<Particle*> Firework::explode() {
	...
	for (int i = 0; i < 100; i++) {
		...
		//srand(time(NULL));
		Particle* prt = new Particle(posit.p, velc*2, 0.5, { 0,-1,0 }, { 0,1,1,1 }, 1,0,false);
			fir.push_back(prt);
